...
if (filter_input(INPUT_POST, "submit") == "Rejestruj") {
    $sql = $um->reg($db);
    if ($sql !== "") {
        $db->insert($sql);
        $_SESSION["valid-reg"] = true;
    }
}
?>
...
<body>
            ...
            <div class="collapse navbar-collapse" id="navbarResponsive">
                <ul class="navbar-nav ml-auto">
                    <li class="nav-item mx-0 mx-lg-1">
                        <a class="nav-link py-3 px-0 px-lg-3" href="Login.php">Logowanie</a>
                    </li>
                    <li class="nav-item mx-0 mx-lg-1">
                        <a class="nav-link py-3 px-0 px-lg-3 active" href="Registration.php">Rejestracja</a>
                    </li>
                </ul>
            </div>
        </div>
    ...
    <section id="mainContent">
        <span class="main-content-text">Stwórz nowe konto</span><br />

        <?php
        $um->regForm();

        if (isset($_SESSION["form-same-userName"])) {
            echo "<script>errorUserName();</script>";
            unset($_SESSION["form-same-userName"]);
        }

        if (isset($_SESSION["form-same-email"])) {
            echo "<script>errorEmail();</script>";
            unset($_SESSION["form-same-email"]);
        }

        if (isset($_SESSION["valid-reg"])) {
            echo "<script>validReg();</script>";
            unset($_SESSION["valid-reg"]);
        }
